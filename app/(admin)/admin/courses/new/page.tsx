...
export default function CreateCoursePage() {
      ...
      for (const lesson of moduleItem.lessons) {
        await supabase
          ...
          .insert({
            ...
            is_live_session: lesson.is_live_session,
            live_start_time: lesson.live_start_time,
  ...
  const getInputValue = (field: FormField, form: CourseForm): string => {
    return form[field.name] as string;
  };
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="mb-2 md:col-span-2">
          ...
          >
            ...
            />
            <span className="text-xs text-gray-500 mt-2">Recommended: 1280x720p, WebP, Max 100KB</span>
            {form.poster && (
              <button
                type="button"
                className="absolute top-2 right-2 bg-white border border-gray-200 rounded-full p-1 shadow hover:bg-gray-100"
                onClick={e => { e.stopPropagation(); setForm(prev => ({ ...prev, poster: "" })); }}
              >
                ✕
              </button>
            )}
        ...
        <div className="flex items-center space-x-3 md:col-span-2">
          <Switch id="is_published" checked={form.is_published} onCheckedChange={(val: boolean) => setForm((prev) => ({ ...prev, is_published: val }))} />
